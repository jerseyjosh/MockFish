...
def create_dataloaders(target_piece, dataset):
    ...
    if dataset=='training':
        ...
        training = ChessDataset(DATA_DIR, 'puzzle_training.pickle', target_piece=target_piece)
        trainLoader = DataLoader(training, num_workers=NUM_WORKERS, batch_size=TRAIN_BATCH_SIZE)
        return trainLoader
    elif dataset=='validation':
        print("Loading validation data...")
        validation = ChessDataset(DATA_DIR, 'puzzle_validation.pickle', target_piece=target_piece)
        validLoader = DataLoader(validation, num_workers=NUM_WORKERS, batch_size=VALID_BATCH_SIZE)
        return validLoader
    elif dataset=='testing':
        print("Loading testing data...")
        testing = ChessDataset(DATA_DIR, 'puzzle_testing.pickle', target_piece=target_piece)
...
def mockfish_train(trainLoader, validLoader, ModelClass, save_dir, target_piece='selector'):
    ...
    # save best model
    torch.save(best_model.state_dict(), save_dir + model._get_name() + f"puzzle_{target_piece}_{current_epoch}e_{current_batch}b_{LEARNING_RATE}lr.pth")
    ...
    losses.to_csv(RESULTS_DIR + model._get_name() + f"puzzle_{target_piece}_{LEARNING_RATE}lr_losses.csv", index=False)
    accuracies.to_csv(RESULTS_DIR + model._get_name() + f"puzzle_{target_piece}_{LEARNING_RATE}lr_accuracies.csv", index=False)
